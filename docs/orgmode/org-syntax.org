...
=noexport=, the type =#+TAGS:= group, type as a heading tag,
=:BLOCKED_BY:= as the graph, and =:BLOCKER:= as org-edna (read, never
minted). =normalize= writes the stamp. =check= names a file that lacks
it or carries a future number.
...
| =calendar-id= =ETag= | org-gcal | Google event. Its =:ID:= may be =<event>/<calendar>=, not org-id. |
