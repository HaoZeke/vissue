...
| =hud=                                                                     | Exec =vissue-hud= (=--toggle=, =--show=, =--hide=, =--offline=, =--foreground=) |
...
=vissue hud= execs =vissue-hud= (PATH or =VISSUE_HUD_BIN=). Same attach
story as the board, including claim and note when serve is down or
mismatched. A second =vissue hud= is a no-op when =hud.sock= accepts.
=--toggle= / =--show= / =--hide= send one line on that socket. Default
detach uses =process_group(0)= only (the child can still see SIGHUP);
