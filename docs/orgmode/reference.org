...
| =hud=                                                                     | Rofi picker over ready (=--mode=, =--iced=) |
...
=vissue hud= is a rofi picker over the ready set. It uses the seat
rofi theme. Return opens the heading in =$EDITOR=. Alt+c claims.
Alt+n notes. =--mode list|claims|stale|new= changes the list.
=--iced= execs the optional overlay.
